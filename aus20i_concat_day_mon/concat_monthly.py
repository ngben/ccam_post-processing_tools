...
def get_unique_years(dir_path):
    """Extracts a sorted list of unique years from 1-year monthly files in the directory."""
    years = set()
    # Matches files like _YYYY01-YYYY12.nc
    pattern = re.compile(r'_(\d{4})01-\1(?:12)\.nc')
...
def concatenate_files(start_year, end_year, dir_path, backup_dir):
    ...
    for year in range(start_year, end_year + 1):
        # We look for the specific 1-year monthly format (Jan to Dec)
        # Using a regex to find the matching file to be safe against varying prefixes
        pattern = re.compile(rf'_({year}01-{year}12)\.nc$')
        for filename in os.listdir(dir_path):
    ...
    prefix_match = re.match(r'^(.*?)_\d{6}-\d{6}\.nc$', first_filename)
    if not prefix_match:
    ...
    # Extract accurate start and end months
    first_date_str = re.search(r'_(\d{6})-\d{6}\.nc$', first_filename).group(1)
    last_date_str = re.search(r'_\d{6}-(\d{6})\.nc$', last_filename).group(1)
...
def main():
    ...
    mon_base_dir = os.path.join(BASE_DIR, "mon")

    if not os.path.exists(mon_base_dir):
        print(f"Directory not found: {mon_base_dir}")
        sys.exit(1)

    # Walk through the base directory looking for variables/versions
    for dirpath, dirnames, filenames in os.walk(mon_base_dir):
        ...
        # Find starting years that end in '1' (e.g., 2001, 2011)
        start_years = [y for y in years if str(y).endswith('1')]
        
        # If no years end in 1, start from the earliest year
        if not start_years:
            start_years = [years[0]]
