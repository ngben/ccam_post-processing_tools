...
def process_files(root_dir, dry_run=True):
    ...
    # Regex to extract the date range (e.g., 20270101-20271231)
    date_pattern = re.compile(r'([0-9]{4})[0-9]{4}-([0-9]{4})[0-9]{4}')
    ...
    for dirpath, _, filenames in os.walk(root_dir):
        ...
        for filename in nc_files:
            ...
            if start_year == end_year:
                ...
                # Extract Prefix to find the target multi-year file
                ...
                if target_file:
                    ...
                    if dry_run:
                        print("-" * 48)
                        print(f"DRY RUN: Would merge {os.path.basename(source_file)} into {os.path.basename(target_file)}")
                        print(f"DRY RUN: Would move {os.path.basename(source_file)} to {processed_dir}")
                    else:
                        print("-" * 48)
